...
export default function RutaOptimizada() {
  ...
  const verificarSiFueLiberado = async () => {
    try {
      ...
      if (data.fue_liberado === true && data.cierre_cuadrado_hoy !== true) {
        ...
        setBotonDeshabilitado(false);
        setVisitados({});
        Toast.show({ type: 'success', text1: '✅ Ruta liberada', text2: 'Recargando clientes del día...' });
        await obtenerDatos();
      }
    } catch (error) {
  ...
  const obtenerDatos = async () => {
    try {
      ...
      if (status !== 'granted') {
        Toast.show({ type: 'error', text1: 'Permiso denegado', text2: 'No se puede acceder a la ubicación' });
        setLoading(false);
        setRefreshing(false);
        return;
      }

      const ubicacionActual = await Location.getCurrentPositionAsync({ accuracy: Location.Accuracy.Balanced });
      setUbicacion(ubicacionActual.coords);

      const token = await AsyncStorage.getItem('authToken');
      if (!token) {
        Toast.show({ type: 'error', text1: 'Sin autenticación', text2: 'No se encontró el token de acceso' });
        setLoading(false);
        setRefreshing(false);
        return;
      }
      ...
      if (!res.ok) {
        Toast.show({ type: 'error', text1: 'Error del servidor', text2: `Status: ${res.status}` });
        setClientes([]);
        setLoading(false);
        setRefreshing(false);
        return;
      ...
      if (Array.isArray(data)) {
        ...
        data.forEach((cliente: any) => {
          if (cliente.ya_visitado) visitadosDelBackend[Number(cliente.id)] = true;
        });
      ...
      Toast.show({ type: 'error', text1: 'Error', text2: error?.message || 'No se pudo obtener los datos' });
      setClientes([]);
    } finally {
      setLoading(false);
      setRefreshing(false);
  ...
  const verRutaCliente = (cliente: any) => {
    if (!ubicacion || !cliente.latitud || !cliente.longitud) {
      Toast.show({ type: 'error', text1: 'Sin coordenadas', text2: 'No se puede trazar la ruta sin ubicación' });
      return;
    }
  ...
  const registrarVisita = async () => {
    if (realizoVenta === null) {
      Toast.show({ type: 'error', text1: 'Selección requerida', text2: 'Indica si se realizó venta o no' });
      return;
    }
    if (realizoVenta === false && !motivoNoVenta) {
      Toast.show({ type: 'error', text1: 'Motivo requerido', text2: 'Selecciona el motivo por el que no se realizó venta' });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        Toast.show({ type: 'success', text1: '✅ Visita registrada', text2: realizoVenta ? 'Venta registrada exitosamente' : 'Visita sin venta registrada' });
        setModalVisible(false);
      } else {
        Toast.show({ type: 'error', text1: '❌ Error al registrar', text2: data.message || 'No se pudo guardar la visita' });
      }
    } catch (error) {
      Toast.show({ type: 'error', text1: '🚫 Error de red', text2: 'No se pudo contactar con el servidor' });
    } finally {
      setGuardando(false);
    }
  };
  ...
  const clientesFiltrados = clientes.filter((c: any) => {
    ...
    const matchQ = !q || String(c?.nombre ?? '').toLowerCase().includes(q) || String(c?.telefono ?? '').toLowerCase().includes(q) || String(c?.id ?? '').includes(q);
      ...
      {loading ? (
        ...
        <>
          ...
          <View style={styles.filtersRow}>
            ...
            >
              <Ionicons name={soloNoVisitados ? 'checkbox' : 'square-outline'} size={16} color={soloNoVisitados ? '#fff' : '#374151'} />
              <Text style={[styles.filterChipText, soloNoVisitados && styles.filterChipTextActive]}>Solo no visitados</Text>
            </TouchableOpacity>
            <Text style={styles.searchHint}>Mostrando {clientesFiltrados.length} de {clientes.length}</Text>
          </View>
            ...
            ListEmptyComponent={
              clientes.length > 0 && (busqueda.trim() || soloNoVisitados) ? (
                <View style={styles.empty}>
                  ...
                  <Text style={styles.emptyText}>No encontramos coincidencias con tu búsqueda o filtros.</Text>
                </View>
              ) : (
                <View style={styles.empty}>
                  <Ionicons name="calendar-outline" size={64} color="#D1D5DB" />
                  <Text style={styles.emptyTitle}>Sin clientes hoy</Text>
                  <Text style={styles.emptyText}>No hay clientes asignados para este día de la semana.</Text>
                </View>
              )
            }
            ListFooterComponent={
        ...
        <View style={styles.modalOverlay}>
          <View style={styles.modalContainer}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <Text style={styles.clienteNombre}>Cliente: {clienteSeleccionado?.nombre}</Text>

              <Text style={styles.pregunta}>¿Se realizó venta?</Text>
              <View style={styles.opcionesContainer}>
                <TouchableOpacity style={[styles.opcionBtn, realizoVenta === true && styles.opcionBtnActiva]} onPress={() => { setRealizoVenta(true); setMotivoNoVenta(null); }}>
                  <Ionicons name="checkmark-circle" size={32} color={realizoVenta === true ? '#10B981' : '#9CA3AF'} />
                  <Text style={[styles.opcionTexto, realizoVenta === true && styles.opcionTextoActiva]}>SÍ, se realizó venta</Text>
                </TouchableOpacity>
                <TouchableOpacity style={[styles.opcionBtn, realizoVenta === false && styles.opcionBtnActiva]} onPress={() => setRealizoVenta(false)}>
                  <Ionicons name="close-circle" size={32} color={realizoVenta === false ? '#EF4444' : '#9CA3AF'} />
                  <Text style={[styles.opcionTexto, realizoVenta === false && styles.opcionTextoActiva]}>NO, no se realizó venta</Text>
                </TouchableOpacity>
              </View>
              ...
              {realizoVenta === false && (
                <>
                  ...
                  {MOTIVOS_NO_VENTA.map((motivo) => (
                    <TouchableOpacity key={motivo.value} style={[styles.motivoBtn, motivoNoVenta === motivo.value && styles.motivoBtnActivo]} onPress={() => setMotivoNoVenta(motivo.value as MotivoNoVenta)}>
                      <Ionicons name={motivo.icon as any} size={20} color={motivoNoVenta === motivo.value ? Colors.light.primario : '#6B7280'} />
                      <Text style={[styles.motivoTexto, motivoNoVenta === motivo.value && styles.motivoTextoActivo]}>{motivo.label}</Text>
                    </TouchableOpacity>
                  ))}
                </>
              )}
              ...
              <TextInput style={styles.textArea} placeholder="Ej: Cliente pidió que vuelva la próxima semana..." value={observaciones} onChangeText={setObservaciones} multiline numberOfLines={4} maxLength={500} />
              ...
              <View style={styles.botonesModal}>
                ...
                <TouchableOpacity style={[styles.btnGuardar, (realizoVenta === null || guardando) && { opacity: 0.5 }]} onPress={registrarVisita} disabled={realizoVenta === null || guardando}>
                  {guardando ? <ActivityIndicator size="small" color="#fff" /> : <Text style={styles.btnGuardarTexto}>Guardar Visita</Text>}
                </TouchableOpacity>
              </View>
            </ScrollView>
          </View>
...
const styles = StyleSheet.create({
  ...
  titulo: { fontSize: 20, fontWeight: 'bold', color: Colors.light.primario, marginBottom: 16, textAlign: 'center' },
  info: { fontSize: 14, marginBottom: 12, textAlign: 'center', color: '#6B7280', fontWeight: '600' },
  searchWrap: { flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#F2F2F2', borderRadius: 12, paddingHorizontal: 12, height: 44, borderWidth: 1, borderColor: '#E5E7EB', marginBottom: 8 },
  searchInput: { flex: 1, height: 44, color: '#111827' },
  filtersRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 },
  filterChip: { flexDirection: 'row', alignItems: 'center', gap: 6, paddingHorizontal: 10, paddingVertical: 8, borderRadius: 999, backgroundColor: '#E5E7EB', borderWidth: 1, borderColor: '#D1D5DB' },
  filterChipActive: { backgroundColor: Colors.light.primario, borderColor: Colors.light.primario },
  filterChipText: { fontWeight: '700', fontSize: 12, color: '#374151' },
  filterChipTextActive: { color: '#fff' },
  searchHint: { fontSize: 12, color: '#6B7280', fontWeight: '600' },
  card: { backgroundColor: '#fff', padding: 16, borderRadius: 12, marginBottom: 12, elevation: 2, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 4 },
  nombre: { fontWeight: '800', fontSize: 16, marginBottom: 6, color: '#111827' },
  coordenadas: { fontSize: 13, color: '#6B7280', marginBottom: 12 },
  botonesContainer: { flexDirection: 'column', gap: 8, marginTop: 4 },
  botonRuta: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: Colors.light.primario, padding: 12, borderRadius: 8 },
  botonVenta: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#059669', padding: 12, borderRadius: 8 },
  botonVisita: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, backgroundColor: '#10B981', padding: 12, borderRadius: 8 },
  ...
  modalTitulo: { fontSize: 22, fontWeight: '800', color: Colors.light.primario, marginBottom: 8, textAlign: 'center' },
  clienteNombre: { fontSize: 16, color: '#6B7280', textAlign: 'center', marginBottom: 24 },
  pregunta: { fontSize: 16, fontWeight: '700', color: '#111827', marginTop: 16, marginBottom: 12 },
  opcionesContainer: { gap: 12, marginBottom: 16 },
  opcionBtn: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 16, backgroundColor: '#F3F4F6', borderRadius: 12, borderWidth: 2, borderColor: 'transparent' },
  opcionBtnActiva: { backgroundColor: '#EFF6FF', borderColor: Colors.light.primario },
  opcionTexto: { fontSize: 15, color: '#6B7280', fontWeight: '600' },
  opcionTextoActiva: { color: '#111827', fontWeight: '700' },
  motivoBtn: { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 12, backgroundColor: '#F9FAFB', borderRadius: 10, marginBottom: 8, borderWidth: 1, borderColor: '#E5E7EB' },
  motivoBtnActivo: { backgroundColor: '#EFF6FF', borderColor: Colors.light.primario },
  motivoTexto: { fontSize: 14, color: '#6B7280' },
  motivoTextoActivo: { color: Colors.light.primario, fontWeight: '700' },
  label: { fontSize: 14, fontWeight: '600', color: '#374151', marginTop: 16, marginBottom: 8 },
  textArea: { backgroundColor: '#F9FAFB', borderWidth: 1, borderColor: '#E5E7EB', borderRadius: 10, padding: 12, fontSize: 14, minHeight: 100, textAlignVertical: 'top' },
  contador: { fontSize: 12, color: '#9CA3AF', textAlign: 'right', marginTop: 4 },
  botonesModal: { flexDirection: 'row', gap: 12, marginTop: 24 },
  btnCancelar: { flex: 1, padding: 14, backgroundColor: '#F3F4F6', borderRadius: 10, alignItems: 'center' },
  btnCancelarTexto: { color: '#111827', fontWeight: '700' },
  btnGuardar: { flex: 1, padding: 14, backgroundColor: Colors.light.primario, borderRadius: 10, alignItems: 'center' },
