...
export default function IniciarVenta() {
  ...
  const confirmarVenta = async () => {
    try {
      const token = await AsyncStorage.getItem('authToken');
      const response = await fetch('http://192.168.100.16/api/venta', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={styles.card}>
              ...
              <Text>
                <Ionicons name="cube-outline" /> Cantidad: {item.cantidad}
              </Text>
              <Text>
                <Ionicons name="calendar-outline" /> Caduca: {item.producto.fecha_caducidad}
              </Text>
              <Text>
                <Ionicons name="cash-outline" /> Precio: ${Number(item.producto.precio).toFixed(2)}
              </Text>
          ...
          <View style={styles.modalContainer}>
            ...
            {carrito.length === 0 ? (
              ...
              <>
                {carrito.map((p) => (
                    <Text key={p.id}>
                        {p.producto.nombre} x {p.cantidad} = ${(p.cantidad * Number(p.producto.precio)).toFixed(2)}
                    </Text>
                    ))}
